...
with gr.Blocks(css=css, title="LLM Chat Application") as demo:
    ...
    async function copyText(text, statusId) {
        ...
        try {
            ...
            statusEl.value = "✓ Copied to clipboard";
            setTimeout(() => { statusEl.value = ""; }, 2000);
        } catch (err) {
            statusEl.value = "Failed to copy: " + err;
        }
    }
    </script>
    <div class="container">
    ...
    with gr.Group(elem_classes=["container"]):
        ...
        with gr.Group():
            ...
            status_display = gr.Textbox(
                ...
                interactive=False,
                visible=True,
                elem_id="copy_status"  # Add unique ID for JavaScript
            )
    ...
    def prepare_copy(text):
        """Prepare text for copying and return JavaScript command"""
        if not text:
            return "document.getElementById('copy_status').value = 'Nothing to copy'"
        return f"copyText({json.dumps(text)}, 'copy_status')"
    ...
    copy_output.click(
        ...
        outputs=None,
        js=prepare_copy  # Changed from _js to js
    )
