...
```"""
    ...
    # Handle HTML characters in non-code sections
    formatted_response = formatted_response.replace('<', '`<`').replace('>', '`>`')
    
    return formatted_response, input_text, response_text
...
with gr.Blocks(css=css, title="LLM Chat Application") as demo:
    ...
    with gr.Group(elem_classes=["container"]):
        ...
        with gr.Group():
            gr.HTML('<div class="container"><h3>Input:</h3></div>')
            with gr.Row():
                ...
                copy_input = gr.Button("📋", elem_classes=["copy-btn"])
            ...
            with gr.Row():
                ...
                copy_output = gr.Button("📋", elem_classes=["copy-btn"])
    ...
    submit_btn.click(
        ...
        outputs=[output, input_display, output_clipboard]
    ).success(
        fn=lambda: "",
